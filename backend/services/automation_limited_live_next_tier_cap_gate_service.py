...
LIMITED_LIVE_NEXT_TIER_CAP_GATE_NOTE_OWNER = "automation-ai"
LIMITED_LIVE_NEXT_TIER_CAP_GATE_HISTORY_LIMIT = 12
LIMITED_LIVE_NEXT_TIER_CAP_GATE_NOTE_LIMIT = 250
LIMITED_LIVE_NEXT_TIER_CAP_GATE_MAX_NOTIONAL_CEILING = 500.0
LIMITED_LIVE_NEXT_TIER_CAP_GATE_PAPER_PROFILE = "personal_paper"
LIMITED_LIVE_NEXT_TIER_CAP_GATE_LIVE_PROFILE = "personal_live"

MARKET_TIMEZONE = automation_ai_review_service.MARKET_TIMEZONE

LIMITED_LIVE_NEXT_TIER_CAP_GATE_SETTINGS_DEFAULTS: dict[str, Any] = {
    "limited_live_next_tier_cap_enabled": False,
    "limited_live_next_tier_cap_max_notional": LIMITED_LIVE_NEXT_TIER_CAP_GATE_MAX_NOTIONAL_CEILING,
    "limited_live_next_tier_cap_duration_minutes": 60,
    "limited_live_next_tier_cap_approval_ttl_minutes": 10,
    "limited_live_next_tier_cap_max_session_orders": 1,
...
def _profile_tag(profile_key: str) -> str:
    return "profile-" + str(profile_key or LIMITED_LIVE_NEXT_TIER_CAP_GATE_PAPER_PROFILE).strip().lower().replace(":", "-")


def _issue(key: str, detail: str, *, component: str = "limited_live_next_tier_cap_gate", severity: str = "blocker") -> dict[str, Any]:
...
def normalize_limited_live_next_tier_cap_gate_settings(settings_state: dict[str, Any] | None) -> dict[str, Any]:
    state = dict(settings_state or {})
    return {
        "limited_live_next_tier_cap_enabled": _coerce_bool(
            state.get("limited_live_next_tier_cap_enabled"),
            bool(LIMITED_LIVE_NEXT_TIER_CAP_GATE_SETTINGS_DEFAULTS["limited_live_next_tier_cap_enabled"]),
        ),
        "limited_live_next_tier_cap_max_notional": _coerce_float(
            state.get("limited_live_next_tier_cap_max_notional"),
            float(LIMITED_LIVE_NEXT_TIER_CAP_GATE_SETTINGS_DEFAULTS["limited_live_next_tier_cap_max_notional"]),
            minimum=1.0,
            maximum=LIMITED_LIVE_NEXT_TIER_CAP_GATE_MAX_NOTIONAL_CEILING,
        ),
        "limited_live_next_tier_cap_duration_minutes": _coerce_int(
            state.get("limited_live_next_tier_cap_duration_minutes"),
            int(LIMITED_LIVE_NEXT_TIER_CAP_GATE_SETTINGS_DEFAULTS["limited_live_next_tier_cap_duration_minutes"]),
            minimum=5,
            maximum=240,
        ),
        "limited_live_next_tier_cap_approval_ttl_minutes": _coerce_int(
            state.get("limited_live_next_tier_cap_approval_ttl_minutes"),
            int(LIMITED_LIVE_NEXT_TIER_CAP_GATE_SETTINGS_DEFAULTS["limited_live_next_tier_cap_approval_ttl_minutes"]),
            minimum=1,
            maximum=30,
        ),
        "limited_live_next_tier_cap_max_session_orders": _coerce_int(
            state.get("limited_live_next_tier_cap_max_session_orders"),
            int(LIMITED_LIVE_NEXT_TIER_CAP_GATE_SETTINGS_DEFAULTS["limited_live_next_tier_cap_max_session_orders"]),
            minimum=1,
            maximum=1,
        ),
        "limited_live_next_tier_cap_require_limit": _coerce_bool(
            state.get("limited_live_next_tier_cap_require_limit"),
            bool(LIMITED_LIVE_NEXT_TIER_CAP_GATE_SETTINGS_DEFAULTS["limited_live_next_tier_cap_require_limit"]),
        ),
        "limited_live_next_tier_cap_auto_expand_enabled": False,
    }


def normalize_limited_live_next_tier_cap_gate_runtime(runtime_state: dict[str, Any] | None) -> dict[str, Any]:
    runtime = dict(runtime_state or {})
    last_report = runtime.get("limited_live_next_tier_cap_gate_last_report")
    if not isinstance(last_report, dict):
        last_report = {}
    approval = runtime.get("limited_live_next_tier_cap_gate_approval")
    if not isinstance(approval, dict):
        approval = {}
    allowance = runtime.get("limited_live_next_tier_cap_gate_allowance")
    if not isinstance(allowance, dict):
        allowance = {}
    history = [
        serialize_value(item)
        for item in list(runtime.get("limited_live_next_tier_cap_gate_history") or [])[
            :LIMITED_LIVE_NEXT_TIER_CAP_GATE_HISTORY_LIMIT
        ]
        if isinstance(item, dict)
    ]
    return {
        "limited_live_next_tier_cap_gate_last_report": serialize_value(last_report),
        "limited_live_next_tier_cap_gate_last_note_id": str(
            runtime.get("limited_live_next_tier_cap_gate_last_note_id") or ""
        ).strip()
        or None,
        "limited_live_next_tier_cap_gate_note_session_day": str(
            runtime.get("limited_live_next_tier_cap_gate_note_session_day") or ""
        ).strip()
        or None,
        "limited_live_next_tier_cap_gate_last_run_at": _serialize_datetime(
            _parse_datetime(runtime.get("limited_live_next_tier_cap_gate_last_run_at"))
        ),
        "limited_live_next_tier_cap_gate_approval": serialize_value(approval),
        "limited_live_next_tier_cap_gate_allowance": serialize_value(allowance),
        "limited_live_next_tier_cap_gate_history": history,
        "limited_live_next_tier_cap_gate_last_error": str(
            runtime.get("limited_live_next_tier_cap_gate_last_error") or ""
        ).strip()
        or None,
    }
...
def build_limited_live_next_tier_cap_gate_snapshot(state: dict[str, Any] | None) -> dict[str, Any]:
    settings_snapshot = normalize_limited_live_next_tier_cap_gate_settings((state or {}).get("settings"))
    runtime = normalize_limited_live_next_tier_cap_gate_runtime((state or {}).get("runtime"))
    report = dict(runtime.get("limited_live_next_tier_cap_gate_last_report") or {})
    approval = dict(runtime.get("limited_live_next_tier_cap_gate_approval") or {})
    allowance = dict(runtime.get("limited_live_next_tier_cap_gate_allowance") or {})
    active = _allowance_active(allowance)
    if not report:
        current_cap = _coerce_float(
            dict((state or {}).get("settings") or {}).get("limited_live_cap_expansion_max_notional")
            or dict((state or {}).get("settings") or {}).get("limited_live_rollout_max_notional"),
            250.0,
        )
        report = {
            "status": "not_prepared",
            "label": "Limited-live next-tier cap gate not prepared",
            "profile_key": LIMITED_LIVE_NEXT_TIER_CAP_GATE_PAPER_PROFILE,
            "live_profile_key": LIMITED_LIVE_NEXT_TIER_CAP_GATE_LIVE_PROFILE,
            ...
            "next_tier_cap_active": active,
            "expansion_expires_at": allowance.get("expires_at"),
            "next_tier_cap_expires_at": allowance.get("expires_at"),
            "current_max_notional": current_cap,
            "expanded_max_notional": settings_snapshot["limited_live_next_tier_cap_max_notional"],
            "next_max_notional": settings_snapshot["limited_live_next_tier_cap_max_notional"],
            "caps": {
                "current_max_notional": current_cap,
                "expanded_max_notional": settings_snapshot["limited_live_next_tier_cap_max_notional"],
                "next_max_notional": settings_snapshot["limited_live_next_tier_cap_max_notional"],
                "max_session_orders": settings_snapshot["limited_live_next_tier_cap_max_session_orders"],
                "duration_minutes": settings_snapshot["limited_live_next_tier_cap_duration_minutes"],
                "require_limit": settings_snapshot["limited_live_next_tier_cap_require_limit"],
            },
            ...
            "related_note_id": runtime.get("limited_live_next_tier_cap_gate_last_note_id"),
            "note_id": runtime.get("limited_live_next_tier_cap_gate_last_note_id"),
            "evaluated_at": runtime.get("limited_live_next_tier_cap_gate_last_run_at"),
        }
    report.setdefault("enabled", settings_snapshot["limited_live_next_tier_cap_enabled"])
    ...
    report.setdefault("next_tier_cap_active", active)
    report.setdefault("expansion_expires_at", allowance.get("expires_at"))
    report.setdefault("next_tier_cap_expires_at", allowance.get("expires_at"))
    report.setdefault("next_max_notional", report.get("expanded_max_notional"))
    report.setdefault(
        "caps",
        {
            "expanded_max_notional": settings_snapshot["limited_live_next_tier_cap_max_notional"],
    ...
    report.setdefault("related_note_id", runtime.get("limited_live_next_tier_cap_gate_last_note_id"))
    report.setdefault("note_id", runtime.get("limited_live_next_tier_cap_gate_last_note_id"))
    report.setdefault("last_error", runtime.get("limited_live_next_tier_cap_gate_last_error"))
...
def _row_is_uncontrolled_live(row: Any) -> bool:
    ...
    if profile != LIMITED_LIVE_NEXT_TIER_CAP_GATE_LIVE_PROFILE and intent != "broker_live":
        return False
    known_markers = (
        "live_pilot_soak_id",
...
def _active_cap_expansion_allowance(runtime: dict[str, Any], now: datetime) -> dict[str, Any]:
...
def _preflight_issues(
    *,
    ...
    gate_settings = normalize_limited_live_next_tier_cap_gate_settings(paper_state.get("settings"))
    ...
    cap_expansion_gate_report = dict(runtime.get("limited_live_cap_expansion_gate_last_report") or {})
    cap_expansion_canary = dict(runtime.get("limited_live_cap_expansion_canary_last_report") or {})
    ...
    cap_expansion_allowance = _active_cap_expansion_allowance(runtime, now)

    if not gate_settings["limited_live_next_tier_cap_enabled"]:
        blockers.append(
            _issue(
                "limited_live_next_tier_cap_disabled",
                "Limited-live next-tier cap gate is disabled. Turn on limited_live_next_tier_cap_enabled before preparing approval.",
            )
        )
    if str(report.get("status") or "").strip().lower() != "ready_to_request_next_tier_cap":
        blockers.append(
            _issue(
                "next_tier_cap_report_not_ready",
                f"Limited-live next-tier cap report status is {report.get('status') or 'missing'}; it must be ready before next-tier approval.",
                component="limited_live_next_tier_cap_report",
            )
        )
    elif _report_is_stale(report, now):
        blockers.append(
            _issue(
                "next_tier_cap_report_stale",
                "Limited-live next-tier cap report evidence is stale; run the report again before next-tier approval.",
                component="limited_live_next_tier_cap_report",
            )
        )
    if report.get("blockers"):
        blockers.append(
            _issue(
                "next_tier_cap_report_has_blockers",
                "The latest next-tier cap report still has unresolved blockers.",
                component="limited_live_next_tier_cap_report",
            )
        )
    if str(report.get("broker_live_gate_status") or report.get("broker_gate_status") or "").strip().lower() != "open":
        blockers.append(_issue("broker_live_gate_not_open", "The next-tier cap report does not show an open broker-live gate.", component="broker_live_gate"))
    if str(report.get("safety_lock_status") or "").strip().lower() not in {"clear", "none"}:
        blockers.append(_issue("safety_lock_active", "The next-tier cap report shows an active safety lock.", component="safety_lock"))
    if str(report.get("state_control_status") or "").strip().lower() == "halt" or str(runtime.get("state_control_state") or "").strip().lower() == "halt":
        blockers.append(_issue("state_control_halt", "State-control is halted; next-tier cap cannot proceed.", component="state_control"))
    ...
    if str(cap_expansion_canary.get("status") or "").strip().lower() == "blocked" or cap_expansion_canary.get("blockers"):
        blockers.append(_issue("cap_expansion_canary_blocked", "Limited-live cap expansion canary has unresolved blocker evidence.", component="limited_live_cap_expansion_canary"))
    if str(cap_expansion_gate_report.get("status") or "").strip().lower() == "blocked" or cap_expansion_gate_report.get("blockers"):
    ...
    if not rollout_allowance:
        blockers.append(
            _issue(
                ...
                "A base limited-live rollout allowance must be active before next-tier cap can be activated.",
                component="limited_live_rollout",
            )
        )
    if not cap_expansion_allowance:
        blockers.append(
            _issue(
                "limited_live_cap_expansion_inactive",
                "A limited-live cap expansion allowance must be active before next-tier cap can be activated.",
                component="limited_live_cap_expansion_gate",
            )
        )
    ...
    if _coerce_bool(live_settings.get("kill_switch"), False) or _coerce_bool(dict(paper_state.get("settings") or {}).get("kill_switch"), False):
        blockers.append(_issue("kill_switch_active", "A paper or live kill switch is active. The next-tier cap gate cannot clear safety locks.", component="safety_lock"))
    ...
    current_cap = _coerce_float(
        cap_expansion_allowance.get("max_notional")
        or cap_expansion_allowance.get("expanded_max_notional")
        or dict(cap_expansion_allowance.get("caps") or {}).get("expanded_max_notional")
        or report.get("current_max_notional")
        or dict(paper_state.get("settings") or {}).get("limited_live_cap_expansion_max_notional")
    ...
    report_cap = _coerce_float(
        ...
        gate_settings["limited_live_next_tier_cap_max_notional"],
        minimum=1.0,
    )
    expanded_cap = min(
        gate_settings["limited_live_next_tier_cap_max_notional"],
        report_cap,
        LIMITED_LIVE_NEXT_TIER_CAP_GATE_MAX_NOTIONAL_CEILING,
    )
    ...
    cap_summary = {
        ...
        "next_max_notional": expanded_cap,
        "report_recommended_max_notional": report_cap,
        "base_rollout_id": rollout_allowance.get("rollout_id"),
        "base_cap_expansion_id": cap_expansion_allowance.get("expansion_id"),
    }
...
def _find_existing_note_id(profile_key: str, session_day: str) -> str | None:
    try:
        payload = notes_service.list_notes(
            status="all",
            tag="automation-ai",
            owner=LIMITED_LIVE_NEXT_TIER_CAP_GATE_NOTE_OWNER,
            limit=LIMITED_LIVE_NEXT_TIER_CAP_GATE_NOTE_LIMIT,
...
def _build_note_body(*, tenant: Tenant, profile_key: str, report: dict[str, Any]) -> str:
    ...
    lines = [
        f"Automation limited-live next-tier cap gate for {tenant.name or tenant.slug} / {profile_key}",
        ...
        f"Next-tier allowance active: {'yes' if report.get('next_tier_cap_active') or report.get('expansion_active') else 'no'}",
        f"Next-tier allowance expires: {report.get('next_tier_cap_expires_at') or report.get('expansion_expires_at') or '--'}",
        f"Current cap: ${float(caps.get('current_max_notional') or report.get('current_max_notional') or 0.0):.2f}",
        f"Next cap: ${float(caps.get('next_max_notional') or report.get('next_max_notional') or caps.get('expanded_max_notional') or report.get('expanded_max_notional') or 0.0):.2f}",
        ...
        "",
        "Prepare only creates short-lived approval. Activate only creates a runtime next-tier-cap allowance. Rollback only disables that allowance. This gate does not place, cancel, or close orders, enable or arm automation, clear locks, tune settings, change broker-live gates, or permit autonomous expansion.",
        "",
        "Blockers",
    ]
...
def _sync_note(*, tenant: Tenant, profile_key: str, report: dict[str, Any]) -> str | None:
    ...
    tags = [
        ...
        "limited-live-rollout",
        _profile_tag(profile_key),
        _profile_tag(LIMITED_LIVE_NEXT_TIER_CAP_GATE_LIVE_PROFILE),
        f"session-{session_day}",
    ]
    title = f"Automation limited-live next-tier cap gate - {profile_key} - {session_day}"
...
def _finalize_report(
    ...
    *,
    ...
    report["label"] = {
        "approved": "Limited-live next-tier cap approved",
        "active": "Limited-live next-tier cap active",
        "rolled_back": "Limited-live next-tier cap rolled back",
        "warning": "Limited-live next-tier cap warning",
        "blocked": "Limited-live next-tier cap blocked",
    }.get(status, "Limited-live next-tier cap gate")
    ...
    summary_keys = {
        ...
        "next_tier_cap_id",
        "next_tier_cap_active",
        "next_tier_cap_expires_at",
        "base_rollout_id",
        "base_cap_expansion_id",
    ...
    runtime["limited_live_next_tier_cap_gate_last_report"] = serialize_value(
        {key: deepcopy(report.get(key)) for key in summary_keys if key in report}
        | {"steps": serialize_value(list(report.get("steps") or [])[:12])}
    )
    runtime["limited_live_next_tier_cap_gate_last_note_id"] = note_id
    runtime["limited_live_next_tier_cap_gate_note_session_day"] = report.get("session_day")
    runtime["limited_live_next_tier_cap_gate_last_run_at"] = report.get("evaluated_at")
    history = list(runtime.get("limited_live_next_tier_cap_gate_history") or [])
    history.insert(0, serialize_value(runtime["limited_live_next_tier_cap_gate_last_report"]))
    runtime["limited_live_next_tier_cap_gate_history"] = history[:LIMITED_LIVE_NEXT_TIER_CAP_GATE_HISTORY_LIMIT]
    runtime["limited_live_next_tier_cap_gate_last_error"] = None
    if db is not None:
        record_audit_event(
            db,
            ...
            payload={
                ...
                "next_tier_cap_id": report.get("next_tier_cap_id") or report.get("expansion_id"),
...
def prepare_limited_live_next_tier_cap(
    db: Session | None,
    *,
    tenant: Tenant,
    paper_state: dict[str, Any],
    ...
    expires_at = now + timedelta(minutes=int(gate_settings["limited_live_next_tier_cap_approval_ttl_minutes"]))
    expansion_expires_at = now + timedelta(minutes=int(gate_settings["limited_live_next_tier_cap_duration_minutes"]))
    caps = {
        "current_max_notional": cap_summary["current_max_notional"],
        "expanded_max_notional": cap_summary["expanded_max_notional"],
        "next_max_notional": cap_summary["next_max_notional"],
        "max_session_orders": int(gate_settings["limited_live_next_tier_cap_max_session_orders"]),
        "duration_minutes": int(gate_settings["limited_live_next_tier_cap_duration_minutes"]),
        "require_limit": bool(gate_settings["limited_live_next_tier_cap_require_limit"]),
    }
    report: dict[str, Any] = {
        "status": "running",
        "profile_key": LIMITED_LIVE_NEXT_TIER_CAP_GATE_PAPER_PROFILE,
        ...
        "next_tier_cap_report_status": cap_report.get("status"),
        ...
        "next_tier_cap_id": None,
        "next_tier_cap_active": False,
        "expansion_expires_at": _serialize_datetime(expansion_expires_at),
        "next_tier_cap_expires_at": _serialize_datetime(expansion_expires_at),
        "base_rollout_id": cap_summary.get("base_rollout_id"),
        "base_cap_expansion_id": cap_summary.get("base_cap_expansion_id"),
        "current_max_notional": caps["current_max_notional"],
        "expanded_max_notional": caps["expanded_max_notional"],
        "next_max_notional": caps["next_max_notional"],
        "caps": caps,
    ...
    if blockers:
        _safe_step(report, "preflight", "blocked", "Limited-live next-tier cap approval was blocked before activation was reachable.")
    else:
        _safe_step(report, "preflight", "approved", "Limited-live next-tier cap approval is fresh; activation remains a separate manual action.")
        paper_state.setdefault("runtime", {})["limited_live_next_tier_cap_gate_approval"] = {
            "approval_id": approval_id,
            "status": "approved",
    ...
    return _finalize_report(
        ...
        audit_event_type="trade_automation.limited_live_next_tier_cap_prepared",
    )


def activate_limited_live_next_tier_cap(
    db: Session | None,
    *,
    tenant: Tenant,
    paper_state: dict[str, Any],
    ...
    runtime = paper_state.setdefault("runtime", {})
    approval = dict(runtime.get("limited_live_next_tier_cap_gate_approval") or {})
    active_allowance = dict(runtime.get("limited_live_next_tier_cap_gate_allowance") or {})
    blockers, warnings, gate_settings, cap_report, rollout_allowance, cap_summary = _preflight_issues(
        paper_state=paper_state,
        live_state=live_state,
        rollout_readiness=rollout_readiness,
    ...
    if not approval_id or approval_status != "approved":
        blockers.append(_issue("approval_missing", "Prepare limited-live next-tier cap before activation."))
    elif expires_at is None or expires_at < now:
        blockers.append(_issue("approval_expired", "The limited-live next-tier cap approval expired. Prepare it again."))
    elif approval.get("consumed_at"):
        blockers.append(_issue("approval_consumed", "The limited-live next-tier cap approval was already consumed. Prepare a fresh approval."))
    if _allowance_active(active_allowance, now):
        blockers.append(_issue("next_tier_cap_already_active", "A limited-live next-tier cap allowance is already active. Roll it back before activating a new one."))
    ...
    expansion_expires_at = now + timedelta(minutes=int(caps.get("duration_minutes") or gate_settings["limited_live_next_tier_cap_duration_minutes"]))
    report: dict[str, Any] = {
        "status": "running",
        "profile_key": LIMITED_LIVE_NEXT_TIER_CAP_GATE_PAPER_PROFILE,
        "live_profile_key": LIMITED_LIVE_NEXT_TIER_CAP_GATE_LIVE_PROFILE,
        ...
        "expansion_active": False,
        "next_tier_cap_id": expansion_id,
        "next_tier_cap_active": False,
        "expansion_expires_at": _serialize_datetime(expansion_expires_at),
        "next_tier_cap_expires_at": _serialize_datetime(expansion_expires_at),
        "base_rollout_id": approval.get("base_rollout_id") or cap_summary.get("base_rollout_id"),
        "base_cap_expansion_id": approval.get("base_cap_expansion_id") or cap_summary.get("base_cap_expansion_id"),
        "current_max_notional": caps.get("current_max_notional"),
        "expanded_max_notional": caps.get("expanded_max_notional"),
        "next_max_notional": caps.get("next_max_notional") or caps.get("expanded_max_notional"),
        "caps": caps,
    ...
    if blockers:
        _safe_step(report, "activate", "blocked", "Limited-live next-tier cap activation was blocked before any runtime allowance was written.")
    else:
        allowance = {
            "expansion_id": expansion_id,
            "next_tier_cap_id": expansion_id,
            ...
            "expanded_max_notional": caps.get("expanded_max_notional"),
            "next_max_notional": caps.get("next_max_notional") or caps.get("expanded_max_notional"),
            "max_notional": caps.get("next_max_notional") or caps.get("expanded_max_notional"),
            "max_session_orders": caps.get("max_session_orders"),
        ...
        runtime["limited_live_next_tier_cap_gate_allowance"] = serialize_value(allowance)
        approval["consumed_at"] = _serialize_datetime(now)
        approval["activated_expansion_id"] = expansion_id
        runtime["limited_live_next_tier_cap_gate_approval"] = serialize_value(approval)
        report["expansion_active"] = True
        report["next_tier_cap_active"] = True
        report["approval_state"] = "consumed"
        _safe_step(report, "activate", "active", "Runtime-only limited-live next-tier cap allowance is active inside hard caps.")
    return _finalize_report(
        db,
        tenant=tenant,
        state=paper_state,
        ...
        audit_event_type="trade_automation.limited_live_next_tier_cap_activated",
    )


def rollback_limited_live_next_tier_cap(
    db: Session | None,
    *,
    tenant: Tenant,
    paper_state: dict[str, Any],
    ...
    report = {
        ...
        "expansion_active": False,
        "next_tier_cap_id": allowance.get("next_tier_cap_id") or allowance.get("expansion_id"),
        "next_tier_cap_active": False,
        "expansion_expires_at": allowance.get("expires_at"),
        "next_tier_cap_expires_at": allowance.get("expires_at"),
        "base_rollout_id": allowance.get("base_rollout_id"),
        "base_cap_expansion_id": allowance.get("base_cap_expansion_id"),
        ...
        "next_max_notional": caps.get("next_max_notional") or allowance.get("next_max_notional") or caps.get("expanded_max_notional") or allowance.get("expanded_max_notional"),
        "caps": caps,
    ...
    _safe_step(report, "rollback", "rolled_back", "Runtime-only limited-live next-tier cap allowance was disabled.")
    return _finalize_report(
        db,
        tenant=tenant,
        state=paper_state,
        ...
        audit_event_type="trade_automation.limited_live_next_tier_cap_rolled_back",
    )
...
def evaluate_limited_live_next_tier_cap_entry_gate(
    ...
    *,
    ...
    settings_snapshot = normalize_limited_live_next_tier_cap_gate_settings(state.get("settings"))
    base_gate = dict(base_gate_result or {})
    base_cap = _base_gate_max_notional(base_gate)
    if not bool(base_gate.get("allowed")):
        return {
            "allowed": False,
            "reason": base_gate.get("reason") or "limited_live_cap_expansion_gate_locked",
            "detail": base_gate.get("detail") or "Limited-live cap expansion allowance must be active before next-tier cap.",
            "expansion_active": False,
            "next_tier_cap_active": False,
    ...
    allowance = dict(runtime.get("limited_live_next_tier_cap_gate_allowance") or {})
    expansion_active = bool(settings_snapshot["limited_live_next_tier_cap_enabled"] and _allowance_active(allowance, now))
    if not expansion_active:
        if estimated_notional is not None and float(estimated_notional) > base_cap + 0.01:
            return {
                "allowed": False,
                "reason": "limited_live_next_tier_cap_required",
                "detail": f"Estimated live entry notional exceeds the base limited-live cap of ${base_cap:.2f}; activate next-tier cap before routing a larger entry.",
                "expansion_active": False,
                "next_tier_cap_active": False,
        ...
        return {
            "allowed": True,
            "reason": "limited_live_cap_expansion_cap_active",
            "detail": "Current expanded limited-live cap applies because no active next-tier cap allowance is present.",
            "expansion_active": False,
            "next_tier_cap_active": False,
            "max_notional": base_cap,
    ...
    if str(allowance.get("session_day") or "").strip() != session_day:
        return {
            "allowed": False,
            "reason": "limited_live_next_tier_cap_session_mismatch",
            "detail": "The active next-tier cap allowance belongs to a different New York session.",
            "expansion_active": False,
            "next_tier_cap_active": False,
    ...
    if consumed >= max_orders:
        return {
            "allowed": False,
            "reason": "limited_live_next_tier_cap_order_cap_exhausted",
            "detail": "The limited-live next-tier cap session order cap is exhausted.",
    ...
    if require_limit and normalized_order_type and normalized_order_type != "limit":
        return {
            "allowed": False,
            "reason": "limited_live_next_tier_cap_limit_required",
            "detail": "Limited-live next-tier cap entries must use limit routing.",
            "expansion_active": True,
            "next_tier_cap_active": True,
            "allowance": serialize_value(allowance),
            "max_notional": base_cap,
    ...
    if estimated_notional is not None and float(estimated_notional) > expanded_cap + 0.01:
        return {
            "allowed": False,
            "reason": "limited_live_next_tier_cap_notional_cap",
            "detail": f"Estimated live entry notional exceeds the next-tier limited-live cap of ${expanded_cap:.2f}.",
            "expansion_active": True,
            "next_tier_cap_active": True,
            "estimated_notional": float(estimated_notional),
    ...
    if str(runtime.get("state_control_state") or "").strip().lower() == "halt" or _coerce_bool(runtime.get("state_control_halt_active"), False):
        return {
            "allowed": False,
            "reason": "limited_live_next_tier_cap_state_control_halt",
            "detail": "State-control halt disables the active next-tier cap allowance until a new approval cycle.",
            "expansion_active": True,
            "next_tier_cap_active": True,
            "allowance": serialize_value(allowance),
            "max_notional": base_cap,
    ...
    return {
        "allowed": True,
        "reason": "limited_live_next_tier_cap_active",
        "detail": "Active next-tier cap allowance permits one higher capped limit live entry.",
        "expansion_active": True,
        "next_tier_cap_active": True,
        "allowance": serialize_value(allowance),
        "next_tier_cap_id": allowance.get("next_tier_cap_id") or allowance.get("expansion_id"),
...
def apply_limited_live_next_tier_cap_entry_overlay(
...
def record_limited_live_next_tier_cap_order_use(
    db: Session | None,
    *,
    tenant: Tenant,
    state: dict[str, Any],
    profile_key: str = LIMITED_LIVE_NEXT_TIER_CAP_GATE_PAPER_PROFILE,
    linked_account: BrokerageLinkedAccount | None = None,
    actor: Any = None,
    now: datetime | None = None,
    ...
    runtime["limited_live_next_tier_cap_gate_allowance"] = serialize_value(allowance)
    last_report = dict(runtime.get("limited_live_next_tier_cap_gate_last_report") or {})
    ...
    last_report.update(
        {
            ...
            "next_tier_cap_active": _allowance_active(allowance, now),
            "expansion_id": allowance.get("expansion_id"),
            "next_tier_cap_id": allowance.get("next_tier_cap_id") or allowance.get("expansion_id"),
            "expansion_expires_at": allowance.get("expires_at"),
            "next_tier_cap_expires_at": allowance.get("expires_at"),
    ...
    if note_id:
        ...
        runtime["limited_live_next_tier_cap_gate_last_note_id"] = note_id
    runtime["limited_live_next_tier_cap_gate_last_report"] = serialize_value(last_report)
    runtime["limited_live_next_tier_cap_gate_last_run_at"] = last_report.get("evaluated_at")
    if db is not None:
        record_audit_event(
            db,
            event_type="trade_automation.limited_live_next_tier_cap_order_recorded",
            tenant=tenant,
            user=actor,
            payload={
                "profile_key": profile_key,
...
def disable_limited_live_next_tier_cap_allowance_for_fault(
    state: dict[str, Any],
    *,
    now: datetime | None = None,
    reason: str,
    ...
    last_report.update(
        {
            ...
            + [_issue("limited_live_next_tier_cap_disabled_for_fault", "Runtime next-tier cap allowance was disabled after a hard safety fault.")],
