...
LIMITED_LIVE_ROLLOUT_CANARY_NOTE_OWNER = "automation-ai"
...
LIMITED_LIVE_ROLLOUT_CANARY_HISTORY_LIMIT = 8
LIMITED_LIVE_ROLLOUT_CANARY_NOTE_LIMIT = 250
LIMITED_LIVE_ROLLOUT_CANARY_PAPER_PROFILE = "personal_paper"
LIMITED_LIVE_ROLLOUT_CANARY_LIVE_PROFILE = "personal_live"
LIMITED_LIVE_ROLLOUT_CANARY_BLOCK_SLIPPAGE_BPS = 100.0
LIMITED_LIVE_ROLLOUT_CANARY_WARN_SLIPPAGE_BPS = 50.0

MARKET_TIMEZONE = automation_ai_review_service.MARKET_TIMEZONE

LIMITED_LIVE_ROLLOUT_CANARY_SETTINGS_DEFAULTS: dict[str, Any] = {
    "limited_live_rollout_canary_enabled": True,
    "limited_live_rollout_canary_auto_review_enabled": True,
    "limited_live_rollout_canary_window_sessions": LIMITED_LIVE_ROLLOUT_CANARY_WINDOW_SESSIONS,
    "limited_live_rollout_canary_required_clean_sessions": LIMITED_LIVE_ROLLOUT_CANARY_REQUIRED_CLEAN_SESSIONS,
    "limited_live_rollout_canary_stale_after_days": LIMITED_LIVE_ROLLOUT_CANARY_STALE_AFTER_DAYS,
}
...
def _profile_tag(profile_key: str) -> str:
    return "profile-" + str(profile_key or LIMITED_LIVE_ROLLOUT_CANARY_PAPER_PROFILE).strip().lower().replace(":", "-")
...
def limited_live_rollout_canary_session_day_for(
...
def next_eligible_limited_live_rollout_canary_review_at(value: datetime | None = None) -> datetime:
    now = value or _utc_now()
    session_day, review_window_open = limited_live_rollout_canary_session_day_for(now, forced=False)
    if review_window_open:
        return now
    return _review_time_for_session_day(session_day)


def next_eligible_limited_live_rollout_canary_review_after_session(session_day: str) -> datetime:
...
def normalize_limited_live_rollout_canary_settings(settings_state: dict[str, Any] | None) -> dict[str, Any]:
    state = dict(settings_state or {})
    window_sessions = _clamp_int(
        state.get("limited_live_rollout_canary_window_sessions"),
        int(LIMITED_LIVE_ROLLOUT_CANARY_SETTINGS_DEFAULTS["limited_live_rollout_canary_window_sessions"]),
        minimum=1,
        maximum=20,
    )
    required_clean_sessions = _clamp_int(
        state.get("limited_live_rollout_canary_required_clean_sessions"),
        int(LIMITED_LIVE_ROLLOUT_CANARY_SETTINGS_DEFAULTS["limited_live_rollout_canary_required_clean_sessions"]),
        minimum=1,
        maximum=window_sessions,
    )
    return {
        "limited_live_rollout_canary_enabled": _coerce_bool(
            state.get("limited_live_rollout_canary_enabled"),
            bool(LIMITED_LIVE_ROLLOUT_CANARY_SETTINGS_DEFAULTS["limited_live_rollout_canary_enabled"]),
        ),
        "limited_live_rollout_canary_auto_review_enabled": _coerce_bool(
            state.get("limited_live_rollout_canary_auto_review_enabled"),
            bool(LIMITED_LIVE_ROLLOUT_CANARY_SETTINGS_DEFAULTS["limited_live_rollout_canary_auto_review_enabled"]),
        ),
        "limited_live_rollout_canary_window_sessions": window_sessions,
        "limited_live_rollout_canary_required_clean_sessions": required_clean_sessions,
        "limited_live_rollout_canary_stale_after_days": _clamp_int(
            state.get("limited_live_rollout_canary_stale_after_days"),
            int(LIMITED_LIVE_ROLLOUT_CANARY_SETTINGS_DEFAULTS["limited_live_rollout_canary_stale_after_days"]),
            minimum=1,
            maximum=30,
        ),
    }
...
def normalize_limited_live_rollout_canary_runtime(runtime_state: dict[str, Any] | None) -> dict[str, Any]:
    runtime = dict(runtime_state or {})
    last_report = runtime.get("limited_live_rollout_canary_last_report")
    if not isinstance(last_report, dict):
        last_report = {}
    history = [
        serialize_value(item)
        for item in list(runtime.get("limited_live_rollout_canary_history") or [])[:LIMITED_LIVE_ROLLOUT_CANARY_HISTORY_LIMIT]
        if isinstance(item, dict)
    ]
    return {
        "limited_live_rollout_canary_last_report": serialize_value(last_report),
        "limited_live_rollout_canary_last_note_id": str(runtime.get("limited_live_rollout_canary_last_note_id") or "").strip()
        or None,
        "limited_live_rollout_canary_note_session_day": str(
            runtime.get("limited_live_rollout_canary_note_session_day") or ""
        ).strip()
        or None,
        "limited_live_rollout_canary_last_run_at": _serialize_datetime(
            _parse_datetime(runtime.get("limited_live_rollout_canary_last_run_at"))
        ),
        "limited_live_rollout_canary_last_scheduled_run_at": _serialize_datetime(
            _parse_datetime(runtime.get("limited_live_rollout_canary_last_scheduled_run_at"))
        ),
        "limited_live_rollout_canary_last_scheduled_session_day": str(
            runtime.get("limited_live_rollout_canary_last_scheduled_session_day") or ""
        ).strip()
        or None,
        "limited_live_rollout_canary_next_eligible_run_at": (
            _serialize_datetime(_parse_datetime(runtime.get("limited_live_rollout_canary_next_eligible_run_at")))
            or _serialize_datetime(next_eligible_limited_live_rollout_canary_review_at())
        ),
        "limited_live_rollout_canary_last_skipped_reason": str(
            runtime.get("limited_live_rollout_canary_last_skipped_reason") or ""
        ).strip()
        or None,
        "limited_live_rollout_canary_last_error": str(
            runtime.get("limited_live_rollout_canary_last_error") or ""
        ).strip()
        or None,
        "limited_live_rollout_canary_history": history,
    }


def build_limited_live_rollout_canary_snapshot(state: dict[str, Any] | None) -> dict[str, Any]:
    runtime = normalize_limited_live_rollout_canary_runtime((state or {}).get("runtime"))
    settings = normalize_limited_live_rollout_canary_settings((state or {}).get("settings"))
    report = dict(runtime.get("limited_live_rollout_canary_last_report") or {})
    if not report:
        return {
            "status": "not_run",
            "label": "Not run",
            "enabled": settings["limited_live_rollout_canary_enabled"],
            "auto_review_enabled": settings["limited_live_rollout_canary_auto_review_enabled"],
            "clean_session_count": 0,
            "required_clean_sessions": settings["limited_live_rollout_canary_required_clean_sessions"],
            "window_session_count": 0,
            "window_days": settings["limited_live_rollout_canary_window_sessions"],
            "latest_rollout_status": "missing",
            ...
            "related_note_id": runtime.get("limited_live_rollout_canary_last_note_id"),
            "note_id": runtime.get("limited_live_rollout_canary_last_note_id"),
            "last_run_at": runtime.get("limited_live_rollout_canary_last_run_at"),
            "last_scheduled_run_at": runtime.get("limited_live_rollout_canary_last_scheduled_run_at"),
            "last_scheduled_session_day": runtime.get("limited_live_rollout_canary_last_scheduled_session_day"),
            "next_eligible_run_at": runtime.get("limited_live_rollout_canary_next_eligible_run_at"),
            "skipped_reason": runtime.get("limited_live_rollout_canary_last_skipped_reason"),
            "last_error": runtime.get("limited_live_rollout_canary_last_error"),
            "sessions": [],
        }
    report.setdefault("enabled", settings["limited_live_rollout_canary_enabled"])
    report.setdefault("auto_review_enabled", settings["limited_live_rollout_canary_auto_review_enabled"])
    report.setdefault("window_days", settings["limited_live_rollout_canary_window_sessions"])
    report.setdefault("required_clean_sessions", settings["limited_live_rollout_canary_required_clean_sessions"])
    report.setdefault("stale_after_days", settings["limited_live_rollout_canary_stale_after_days"])
    report.setdefault("related_note_id", runtime.get("limited_live_rollout_canary_last_note_id"))
    report.setdefault("note_id", runtime.get("limited_live_rollout_canary_last_note_id"))
    report.setdefault("last_run_at", runtime.get("limited_live_rollout_canary_last_run_at"))
    report.setdefault("last_scheduled_run_at", runtime.get("limited_live_rollout_canary_last_scheduled_run_at"))
    report.setdefault("last_scheduled_session_day", runtime.get("limited_live_rollout_canary_last_scheduled_session_day"))
    report.setdefault("next_eligible_run_at", runtime.get("limited_live_rollout_canary_next_eligible_run_at"))
    report.setdefault("skipped_reason", runtime.get("limited_live_rollout_canary_last_skipped_reason"))
    report.setdefault("last_error", runtime.get("limited_live_rollout_canary_last_error"))
...
def _issue(
    ...
    detail: str,
    *,
    component: str = "limited_live_rollout_canary",
...
def _runtime_report_by_day(runtime: dict[str, Any], session_day: str) -> dict[str, Any]:
    last = runtime.get("limited_live_rollout_gate_last_report")
    if isinstance(last, dict) and str(last.get("session_day") or "").strip() == session_day:
        return dict(last)
    for item in list(runtime.get("limited_live_rollout_gate_history") or []):
...
def _note_lookup(profile_key: str) -> dict[str, dict[str, Any]]:
    try:
        payload = notes_service.list_notes(
            status="all",
            tag="automation-ai",
            owner=LIMITED_LIVE_ROLLOUT_CANARY_NOTE_OWNER,
            limit=LIMITED_LIVE_ROLLOUT_CANARY_NOTE_LIMIT,
    ...
    for item in list(payload.get("items") or []):
        ...
        if "limited-live-rollout-canary" in tags:
            bucket["limited_live_rollout_canary"] = item
        if "limited-live-rollout" in tags:
            bucket["limited_live_rollout"] = item
        if "live-pilot-promotion" in tags:
            bucket["live_pilot_promotion"] = item
    return by_day
...
def _recent_order_events(
    ...
    *,
    ...
    for row in rows:
        ...
        if row_profile and row_profile != LIMITED_LIVE_ROLLOUT_CANARY_LIVE_PROFILE:
            continue
        if not rollout_id and route_family != "limited_live_rollout" and entry_reason != "limited_live_rollout":
            continue
        events.append(
            {
...
def _row_is_uncontrolled_live(row: Any) -> bool:
    ...
    if profile != LIMITED_LIVE_ROLLOUT_CANARY_LIVE_PROFILE and intent != "broker_live":
        return False
    known_markers = (
        "live_pilot_soak_id",
...
def _promotion_stale(promotion: dict[str, Any], now: datetime, stale_after_days: int) -> bool:
...
def _session_report(
    *,
    ...
    rollout_status = str(rollout_item.get("status") or "").strip().lower()
    evidence = dict(rollout_item.get("candidate_order_evidence") or {})
    orders = [item for item in list(evidence.get("orders") or []) if isinstance(item, dict)]
    consumed_count = max(_coerce_int(rollout_item.get("consumed_order_count"), 0), len(orders))
    caps = dict(rollout_item.get("caps") or {})
    max_orders = _coerce_int(caps.get("max_session_orders") or rollout_item.get("max_session_orders"), 1)
    max_notional = _coerce_float(caps.get("max_notional") or rollout_item.get("notional_cap"), 100.0)
    ...
    reconciliation_status = str(
        rollout_item.get("reconciliation_status")
        or evidence.get("reconciliation_status")
        or ("clean" if consumed_count and not failed_events else "missing")
    ).strip().lower()
    note_id = (
        str(rollout_item.get("related_note_id") or rollout_item.get("note_id") or "").strip()
        or str((notes.get("limited_live_rollout") or {}).get("id") or "").strip()
        or None
    )

    if not rollout_status:
        blockers.append(_issue("limited_live_rollout_missing", "No limited-live rollout gate evidence exists for this session."))
    elif rollout_status in {"blocked", "failed", "fail"} or rollout_item.get("blockers"):
        blockers.append(_issue("limited_live_rollout_blocked", "Limited-live rollout gate evidence has unresolved blockers."))
    elif rollout_status == "warning" or rollout_item.get("warnings"):
        warnings.append(_issue("limited_live_rollout_warning", "Limited-live rollout gate evidence has warnings.", severity="warning"))
    if consumed_count <= 0:
        blockers.append(_issue("no_consumed_order_evidence", "No capped live order was consumed during this limited-live rollout session."))
    if max_orders > 0 and consumed_count > max_orders:
        blockers.append(_issue("session_order_cap_breached", "Consumed limited-live order count exceeded the configured session cap."))
    if require_limit and (non_limit_orders or non_limit_events):
        blockers.append(_issue("non_limit_order_evidence", "Limited-live rollout evidence includes a non-limit order."))
    if order_notionals and max(order_notionals) > max_notional + 0.01:
        blockers.append(_issue("notional_cap_breached", "Limited-live rollout evidence exceeds the configured notional cap."))
    if consumed_count > 0 and not terminal_state:
        blockers.append(_issue("terminal_evidence_missing", "Consumed live order evidence is missing terminal broker/local state."))
    if failed_events:
        blockers.append(_issue("order_event_failed", "Limited-live rollout order events include failed, blocked, or rejected status.", component="order_events"))
    ...
    if not note_id:
        blockers.append(_issue("limited_live_note_missing", "Limited-live rollout session is missing a linked automation-ai note.", component="notes"))
    ...
    if worst_slippage is not None and worst_slippage > LIMITED_LIVE_ROLLOUT_CANARY_BLOCK_SLIPPAGE_BPS:
        blockers.append(
            _issue(
                "slippage_blocked",
                f"Limited-live rollout worst slippage was {worst_slippage:.1f} bps.",
                component="slippage",
            )
        )
    elif worst_slippage is not None and worst_slippage > LIMITED_LIVE_ROLLOUT_CANARY_WARN_SLIPPAGE_BPS:
        warnings.append(
            _issue(
                "slippage_warning",
                f"Limited-live rollout worst slippage was {worst_slippage:.1f} bps.",
    ...
    if sum(pnl_values) < 0:
        warnings.append(_issue("negative_pnl", "Limited-live rollout realized PnL is negative.", component="pnl", severity="warning"))

    promotion_status = str(promotion.get("status") or "").strip().lower() or "missing"
    if promotion_status != "ready_to_request_limited_live_rollout":
    ...
    return serialize_value(
        {
            ...
            "status": status,
            "clean": status == "clean",
            "limited_live_rollout": rollout_item,
...
def _aggregate_report(
    *,
    ...
    settings = normalize_limited_live_rollout_canary_settings(paper_state.get("settings"))
    ...
    window_days = int(settings["limited_live_rollout_canary_window_sessions"])
    required_clean_sessions = int(settings["limited_live_rollout_canary_required_clean_sessions"])
    stale_after_days = int(settings["limited_live_rollout_canary_stale_after_days"])
    ...
    if profile_key != LIMITED_LIVE_ROLLOUT_CANARY_PAPER_PROFILE:
        blockers.insert(0, _issue("paper_profile_required", "V1 limited-live rollout canary is reviewed from the personal paper automation profile."))
    if not sessions:
        blockers.append(_issue("no_limited_live_rollout_sessions", "No limited-live rollout evidence was found in the configured trading-day window."))

    promotion_status = str(promotion.get("status") or "").strip().lower() or "missing"
    broker_gate_status = str(promotion.get("broker_live_gate_status") or "unknown").strip().lower()
    safety_lock_status = str(promotion.get("safety_lock_status") or "unknown").strip().lower()
    if promotion_status in {"missing", "not_run"}:
        blockers.append(_issue("promotion_report_missing", "Run the live pilot promotion report before the limited-live rollout canary.", component="live_pilot_promotion"))
    elif promotion_status != "ready_to_request_limited_live_rollout":
        blockers.append(_issue("promotion_report_not_ready", "Live pilot promotion report is not ready.", component="live_pilot_promotion"))
    elif _promotion_stale(promotion, now, stale_after_days):
    ...
    latest_rollout = dict(latest_session.get("limited_live_rollout") or {})
    ...
    if blockers:
        ...
        status = "warning"
    label = {
        "ready_for_operator_review": "Limited-live rollout canary ready for operator review",
        "collecting": "Collecting limited-live rollout canary evidence",
        "warning": "Limited-live rollout canary warning",
        "blocked": "Limited-live rollout canary blocked",
    }.get(status, "Limited-live rollout canary")
    return serialize_value(
        {
            "status": status,
            "label": label,
            "profile_key": profile_key,
            "live_profile_key": LIMITED_LIVE_ROLLOUT_CANARY_LIVE_PROFILE,
            ...
            "latest_rollout_status": latest_rollout.get("status") or "missing",
...
def _build_note_body(*, tenant: Tenant, profile_key: str, report: dict[str, Any]) -> str:
    ...
    lines = [
        f"Automation limited-live rollout canary for {tenant.name or tenant.slug} / {profile_key}",
        ...
        (
            ...
            if evidence_window
            else f"Evidence window: {report.get('window_days', LIMITED_LIVE_ROLLOUT_CANARY_WINDOW_SESSIONS)} trading session(s)"
        ),
        f"Latest rollout: {report.get('latest_rollout_status') or 'missing'}",
        ...
        "",
        "This canary is advisory. It does not place, cancel, or close orders, enable live trading, arm automation, clear locks, tune baseline settings, activate or rollback allowances, or change broker-live gates.",
        "",
        "Blockers",
    ]
    ...
    for item in list(report.get("sessions") or [])[: _coerce_int(report.get("window_days"), LIMITED_LIVE_ROLLOUT_CANARY_WINDOW_SESSIONS)]:
        rollout = dict(item.get("limited_live_rollout") or {})
        events = dict(item.get("order_events") or {})
        lines.append(
            f"- {item.get('session_day')}: {str(item.get('status') or '').upper()} | "
            f"rollout {rollout.get('status') or 'missing'} | "
...
def _sync_canary_note(*, tenant: Tenant, profile_key: str, report: dict[str, Any]) -> str | None:
    ...
    tags = [
        ...
        "live-pilot-promotion",
        _profile_tag(profile_key),
        _profile_tag(LIMITED_LIVE_ROLLOUT_CANARY_LIVE_PROFILE),
        f"session-{session_day}",
    ]
    title = f"Automation limited-live rollout canary - {profile_key} - {session_day}"
...
def run_limited_live_rollout_canary_review(
    db: Session | None,
    *,
    tenant: Tenant,
    paper_state: dict[str, Any],
    live_state: dict[str, Any] | None = None,
    profile_key: str = LIMITED_LIVE_ROLLOUT_CANARY_PAPER_PROFILE,
    ...
    review_session_day, _review_window_open = limited_live_rollout_canary_session_day_for(
        now,
    ...
    report["last_scheduled_run_at"] = (
        ...
        else _serialize_datetime(_parse_datetime(runtime_before.get("limited_live_rollout_canary_last_scheduled_run_at")))
    )
    report["last_scheduled_session_day"] = (
        review_session_day
        if normalized_run_source == "scheduled"
        else str(runtime_before.get("limited_live_rollout_canary_last_scheduled_session_day") or "").strip() or None
    )
    next_eligible = (
        next_eligible_limited_live_rollout_canary_review_after_session(review_session_day)
        if normalized_run_source == "scheduled"
        else next_eligible_limited_live_rollout_canary_review_at(now)
    )
    ...
    runtime["limited_live_rollout_canary_last_report"] = serialize_value(summary)
    runtime["limited_live_rollout_canary_last_note_id"] = note_id
    runtime["limited_live_rollout_canary_note_session_day"] = report.get("session_day")
    runtime["limited_live_rollout_canary_last_run_at"] = report.get("evaluated_at")
    if normalized_run_source == "scheduled":
        runtime["limited_live_rollout_canary_last_scheduled_run_at"] = report.get("evaluated_at")
        runtime["limited_live_rollout_canary_last_scheduled_session_day"] = review_session_day
    runtime["limited_live_rollout_canary_next_eligible_run_at"] = report.get("next_eligible_run_at")
    runtime["limited_live_rollout_canary_last_skipped_reason"] = None
    runtime["limited_live_rollout_canary_last_error"] = None
    history = list(runtime.get("limited_live_rollout_canary_history") or [])
    history.insert(
        0,
        {
    ...
    runtime["limited_live_rollout_canary_history"] = serialize_value(history[:LIMITED_LIVE_ROLLOUT_CANARY_HISTORY_LIMIT])
    if db is not None:
        record_audit_event(
            db,
            event_type="trade_automation.limited_live_rollout_canary_reviewed",
            tenant=tenant,
            user=actor,
            payload={
                "profile_key": profile_key,
