...
class AutomationLimitedLiveNextTierCapGateServiceTests(unittest.TestCase):
    def _db(self):
        engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
        Base.metadata.create_all(engine)
        SessionLocal = sessionmaker(bind=engine, future=True)
        db = SessionLocal()
        tenant = Tenant(slug="next-tier-cap-gate-test", name="Next Tier Cap Gate Test", status="active")
        db.add(tenant)
        db.commit()
        db.refresh(tenant)
    ...
    def _paper_state(self, *, enabled: bool = True, report_status: str = "ready_to_request_next_tier_cap") -> dict:
        ...
        state["settings"].update(
            {
                ...
                "limited_live_next_tier_cap_enabled": enabled,
        ...
        state["runtime"].update(
            {
                "limited_live_next_tier_cap_report_last_report": {
                    ...
                    "stale_after_days": 2,
                    "current_max_notional": 250.0,
                    "recommended_next_max_notional": 500.0,
                    "target_max_notional": 500.0,
                    "broker_live_gate_status": "open",
                    "safety_lock_status": "clear",
                    "state_control_status": "healthy",
                    "blockers": [],
                    "warnings": [],
                    "related_note_id": "next-tier-report-note",
                },
                ...
                "limited_live_cap_expansion_canary_last_report": {
                    ...
                    "blockers": [],
                    "warnings": [],
                    "related_note_id": "cap-canary-note",
                },
                ...
                "limited_live_cap_expansion_gate_allowance": {
                    "expansion_id": "cap-expansion-1",
                    "approval_id": "cap-approval-1",
                    "base_rollout_id": "rollout-1",
                    "status": "active",
                    "active": True,
                    "session_day": "2026-04-24",
                    "expires_at": "2026-04-24T22:20:00+00:00",
    ...
    def _patch_io(self):
        ...
        return (
            ...
            patch.object(automation_limited_live_next_tier_cap_gate_service.sdm, "read_pending_orders", return_value=pd.DataFrame()),
            patch.object(automation_limited_live_next_tier_cap_gate_service.sdm, "read_open_trades", return_value=pd.DataFrame()),
            notes_path,
        )

    def test_action_schema_accepts_limited_live_next_tier_cap_actions(self) -> None:
        self.assertEqual(
            OrganizationTradeAutomationActionRequest(action="prepare_limited_live_next_tier_cap").action,
            "prepare_limited_live_next_tier_cap",
        )
        self.assertEqual(
            OrganizationTradeAutomationActionRequest(action="activate_limited_live_next_tier_cap").action,
            "activate_limited_live_next_tier_cap",
        )
        self.assertEqual(
            OrganizationTradeAutomationActionRequest(action="rollback_limited_live_next_tier_cap").action,
    ...
    def test_disabled_setting_blocks_prepare_without_allowance(self) -> None:
        ...
        *context_patches, notes_path = patches
        ...
        self.assertIn("limited_live_next_tier_cap_disabled", {item["key"] for item in report["blockers"]})
        self.assertFalse(paper_state["runtime"].get("limited_live_next_tier_cap_gate_allowance"))
        self.assertEqual(len([note for note in notes if "limited-live-next-tier-cap-gate" in note.get("tags", [])]), 1)
    ...
    def test_clean_prepare_writes_short_lived_approval_note_and_audit(self) -> None:
        ...
        *context_patches, notes_path = patches
        ...
        self.assertEqual(float(report["next_max_notional"]), 500.0)
        self.assertEqual(paper_state["settings"], before_settings)
        self.assertEqual(paper_state["runtime"]["limited_live_next_tier_cap_gate_approval"]["status"], "approved")
        self.assertFalse(paper_state["runtime"].get("limited_live_next_tier_cap_gate_allowance"))
        self.assertIn("Prepare only creates short-lived approval", notes[0]["body"])
        self.assertIn(
            "trade_automation.limited_live_next_tier_cap_prepared",
    ...
    def test_activate_requires_fresh_approval_and_creates_runtime_only_allowance(self) -> None:
        ...
        *context_patches, _notes_path = patches
        ...
        with context_patches[0], context_patches[1], context_patches[2], context_patches[3]:
            blocked = automation_limited_live_next_tier_cap_gate_service.activate_limited_live_next_tier_cap(
                db,
                tenant=tenant,
                paper_state=paper_state,
                live_state=self._live_state(),
            ...
            automation_limited_live_next_tier_cap_gate_service.prepare_limited_live_next_tier_cap(
                db,
                tenant=tenant,
                paper_state=paper_state,
                live_state=self._live_state(),
        ...
        allowance = paper_state["runtime"]["limited_live_next_tier_cap_gate_allowance"]
        self.assertEqual(allowance["status"], "active")
        self.assertEqual(allowance["max_session_orders"], 1)
        self.assertEqual(allowance["base_cap_expansion_id"], "cap-expansion-1")
        self.assertLessEqual(float(allowance["max_notional"]), 500.0)
        self.assertEqual(paper_state["settings"]["enabled"], False)
        self.assertIn(
            "trade_automation.limited_live_next_tier_cap_activated",
            [row.event_type for row in db.query(AuditEvent).all()],
        )

    def test_entry_gate_requires_next_tier_for_amounts_above_current_expanded_cap(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        base_gate = {
            "allowed": True,
            "cap_expansion_id": "cap-expansion-1",
            "expansion_active": True,
            "max_notional": 250.0,
        ...
        *context_patches, _notes_path = patches
        ...
        before_activation = automation_limited_live_next_tier_cap_gate_service.evaluate_limited_live_next_tier_cap_entry_gate(
            paper_state,
            base_gate_result=base_gate,
            now=FIXED_NOW + timedelta(minutes=1),
            order_type="limit",
            estimated_notional=350.0,
        )
        ...
        with context_patches[0], context_patches[1], context_patches[2], context_patches[3]:
            ...
            automation_limited_live_next_tier_cap_gate_service.activate_limited_live_next_tier_cap(
                db,
                tenant=tenant,
                paper_state=paper_state,
                live_state=self._live_state(),
        ...
        expanded = automation_limited_live_next_tier_cap_gate_service.evaluate_limited_live_next_tier_cap_entry_gate(
            paper_state,
            base_gate_result=base_gate,
        ...
        too_large = automation_limited_live_next_tier_cap_gate_service.evaluate_limited_live_next_tier_cap_entry_gate(
            paper_state,
            base_gate_result=base_gate,
            now=FIXED_NOW + timedelta(minutes=2),
            order_type="limit",
            estimated_notional=501.0,
        )
        market = automation_limited_live_next_tier_cap_gate_service.evaluate_limited_live_next_tier_cap_entry_gate(
            paper_state,
            base_gate_result=base_gate,
            now=FIXED_NOW + timedelta(minutes=2),
            order_type="market",
        ...
        self.assertEqual(before_activation["reason"], "limited_live_next_tier_cap_required")
        self.assertTrue(expanded["allowed"])
        self.assertEqual(float(expanded["max_notional"]), 500.0)
        self.assertEqual(too_large["reason"], "limited_live_next_tier_cap_notional_cap")
        self.assertEqual(market["reason"], "limited_live_next_tier_cap_limit_required")

    def test_recording_order_consumption_exhausts_next_tier_gate_and_updates_note(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        base_gate = {"allowed": True, "cap_expansion_id": "cap-expansion-1", "max_notional": 250.0}
        patches = self._patch_io()
        *context_patches, notes_path = patches
        ...
        exhausted = automation_limited_live_next_tier_cap_gate_service.evaluate_limited_live_next_tier_cap_entry_gate(
            paper_state,
            base_gate_result=base_gate,
            now=FIXED_NOW + timedelta(minutes=3),
            order_type="limit",
        )
        self.assertEqual(exhausted["reason"], "limited_live_next_tier_cap_order_cap_exhausted")
        self.assertEqual(paper_state["runtime"]["limited_live_next_tier_cap_gate_allowance"]["consumed_order_count"], 1)
    ...
    def test_manual_rollback_disables_runtime_allowance(self) -> None:
        ...
        *context_patches, _notes_path = patches
        ...
        self.assertFalse(paper_state["runtime"]["limited_live_next_tier_cap_gate_allowance"]["active"])
    ...
    def test_action_path_persists_metadata_without_order_path_or_settings_mutation(self) -> None:
        ...
        current_user = SimpleNamespace(
            ...
            auth_subject="next-tier-cap-gate",
            user_id="next-tier-cap-gate",
        ...
        *context_patches, notes_path = patches
        ...
        with (
            ...
            patch.object(automation_limited_live_next_tier_cap_gate_service, "_utc_now", return_value=FIXED_NOW),
        ):
            prepared = trade_automation_service.run_tenant_trade_automation_action(
                db,
                current_user=current_user,
                request=OrganizationTradeAutomationActionRequest(action="prepare_limited_live_next_tier_cap"),
            )
            activated = trade_automation_service.run_tenant_trade_automation_action(
                db,
                current_user=current_user,
                request=OrganizationTradeAutomationActionRequest(action="activate_limited_live_next_tier_cap"),
            )
            rolled_back = trade_automation_service.run_tenant_trade_automation_action(
                db,
                current_user=current_user,
                request=OrganizationTradeAutomationActionRequest(action="rollback_limited_live_next_tier_cap"),
            )
            notes = json.loads(notes_path.read_text(encoding="utf-8"))
        ...
        self.assertEqual(prepared["limited_live_next_tier_cap_gate"]["status"], "approved")
        self.assertEqual(activated["limited_live_next_tier_cap_gate"]["status"], "active")
        self.assertEqual(rolled_back["limited_live_next_tier_cap_gate"]["status"], "rolled_back")
        self.assertEqual(len([note for note in notes if "limited-live-next-tier-cap-gate" in note.get("tags", [])]), 1)
