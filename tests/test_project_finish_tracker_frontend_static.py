...
class ProjectFinishTrackerFrontendStaticTests(unittest.TestCase):
    ...
    def test_shared_component_and_client_fallback_exist(self) -> None:
        ...
        self.assertIn("They do not authorize live trading, AI order authority", component)
        self.assertIn("kill-switch bypass", component)
        self.assertIn("Next safe action", component)
        self.assertIn("next_safe_action", component)
        self.assertIn("Review the proof-roadmap docs before making readiness claims", component)
        self.assertIn("Loading project finish tracker and proof boundaries", component)
        ...
        self.assertIn("FALLBACK_FINISH_TRACKER_ITEMS.map(withNextSafeAction)", client)
        self.assertIn("project_finish_tracker_v2", client)
        self.assertIn("proof_first_rule", client)
        self.assertIn("AI order authority", client)
        self.assertIn("broker-route changes", client)
        self.assertIn("risk-gate bypass", client)
        self.assertIn("ranking-weight mutation", client)
