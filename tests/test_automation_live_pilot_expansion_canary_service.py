...
FIXED_NOW = datetime(2026, 4, 24, 21, 5, tzinfo=timezone.utc)
BEFORE_CLOSE_NOW = datetime(2026, 4, 24, 19, 30, tzinfo=timezone.utc)
SESSION_DAYS = ["2026-04-24", "2026-04-23", "2026-04-22"]


class AutomationLivePilotExpansionCanaryServiceTests(unittest.TestCase):
    def _db(self):
        engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
        Base.metadata.create_all(engine)
        SessionLocal = sessionmaker(bind=engine, future=True)
        db = SessionLocal()
        tenant = Tenant(slug="live-expansion-canary-test", name="Live Expansion Canary Test", status="active")
        db.add(tenant)
        db.commit()
        db.refresh(tenant)
        ...
        state["runtime"].update(
            {
                ...
                "live_pilot_expansion_history": [
                    {
                        "at": f"{day}T20:50:00+00:00",
                        "checked_at": f"{day}T20:50:00+00:00",
                        "session_day": day,
                        "status": "completed",
                        "terminal_state": "canceled",
                        "broker_order_id": f"live-expansion-broker-{day}",
                        "local_order_id": f"live-expansion-local-{day}",
                        "reconciliation_status": "clean",
                        "notional_cap": 25.0,
                        "estimated_notional": 0.48,
                        "selected_candidate": {
                            "ticker": "SPY",
                            "portfolio_rank": 1,
                            "alpha_score": 91,
                        ...
                        "fill_evidence": {"slippage_bps": 8.5},
                        "close_evidence": {"realized_pnl": 0.0},
                        "blockers": [],
                        "warnings": [],
                        "note_id": f"live-expansion-{day}",
                    }
                    for day in SESSION_DAYS
                ],
            }
    ...
    def _seed_notes(self, *, missing: set[tuple[str, str]] | None = None) -> None:
        ...
        for day in SESSION_DAYS:
            ...
            if (day, "expansion") not in missing:
                notes_service.create_note(
                    title=f"Live expansion {day}",
                    body="Operator-approved live expansion completed",
                    tags=[*tag_base, "live-pilot-expansion", "live-pilot-readiness"],
                    owner="automation-ai",
    ...
    def _seed_submit_events(self, db, tenant, *, days: list[str] | None = None, order_type: str = "limit") -> None:
        for day in days or SESSION_DAYS:
            db.add(
                OrderEventRecord(
                    tenant_id=tenant.id,
                    trade_id=f"live-expansion-trade-{day}",
                    ticker="SPY",
                    event_key="order.submitted",
                    status="submitted",
                    order_type=order_type,
                    detail="Live expansion submit evidence.",
                    payload_json={
                        "automation_profile_key": "personal_live",
                        "live_pilot_expansion_id": f"live-expansion-{day}",
    ...
    def test_action_schema_accepts_live_pilot_expansion_canary_review(self) -> None:
        payload = OrganizationTradeAutomationActionRequest(action="run_live_pilot_expansion_canary_review")

        self.assertEqual(payload.action, "run_live_pilot_expansion_canary_review")

    def test_clean_live_expansion_sessions_mark_canary_ready_and_write_note(self) -> None:
        db, tenant = self._db()
        ...
        self.assertEqual(report["latest_expansion_status"], "completed")
        self.assertEqual(report["latest_terminal_state"], "canceled")
        self.assertEqual(report["candidate_evidence"]["ticker"], "SPY")
        self.assertEqual(report["slippage_summary"]["sample_count"], 3)
        canary_notes = [note for note in notes if "live-pilot-expansion-canary" in note.get("tags", [])]
        self.assertEqual(len(canary_notes), 1)
        self.assertIn("does not place or cancel orders", canary_notes[0]["body"])
        self.assertEqual(paper_state["runtime"]["live_pilot_expansion_canary_last_report"]["status"], "ready")
        audit_types = [row.event_type for row in db.query(AuditEvent).all()]
        self.assertIn("trade_automation.live_pilot_expansion_canary_reviewed", audit_types)

    def test_missing_expansion_evidence_blocks_canary(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        paper_state["runtime"]["live_pilot_expansion_history"] = []

        report, _notes = self._run_canary(
            db,
            tenant,
            paper_state,
            notes_missing={(day, "expansion") for day in SESSION_DAYS},
            seed_events=False,
        )

        self.assertEqual(report["status"], "blocked")
        self.assertIn("live_expansion_missing", {item["key"] for item in report["blockers"]})

    def test_failed_cancel_and_reconciliation_blocks_canary(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        paper_state["runtime"]["live_pilot_expansion_history"][0].update(
            {
        ...
        self.assertIn("live_expansion_blocked", blocker_keys)
    ...
    def test_unexpected_fill_closed_cleanly_counts_as_clean(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        for item in paper_state["runtime"]["live_pilot_expansion_history"]:
    ...
    def test_current_readiness_blocked_blocks_canary(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
    ...
    def test_candidate_rejection_blocks_canary(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        paper_state["runtime"]["live_pilot_expansion_history"][0]["selected_candidate"].update(
    ...
    def test_missing_note_and_order_event_block_canary(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        ...
        report, _notes = self._run_canary(
            ...
            tenant,
            paper_state,
            notes_missing={("2026-04-24", "expansion")},
            seed_events=False,
        )

        self.assertEqual(report["status"], "blocked")
        blocker_keys = {item["key"] for item in report["blockers"]}
        self.assertIn("live_expansion_note_missing", blocker_keys)
        self.assertIn("order_count_not_one", blocker_keys)
    ...
    def test_action_path_persists_report_without_order_or_setting_mutation(self) -> None:
        ...
        current_user = SimpleNamespace(
            ...
            auth_subject="live-expansion-canary",
            user_id="live-expansion-canary",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                patch.object(automation_live_pilot_expansion_canary_service, "_utc_now", return_value=FIXED_NOW),
                ...
                patch.object(automation_live_pilot_expansion_service, "prepare_live_pilot_expansion") as prepare_mock,
                ...
                snapshot = trade_automation_service.run_tenant_trade_automation_action(
                    ...
                    request=OrganizationTradeAutomationActionRequest(action="run_live_pilot_expansion_canary_review"),
                )
                notes = json.loads(notes_path.read_text(encoding="utf-8"))
        ...
        self.assertEqual(snapshot["live_pilot_expansion_canary"]["status"], "ready")
        self.assertTrue(snapshot["live_pilot_expansion_canary"]["related_note_id"])
        self.assertTrue(snapshot["available_actions"]["can_run_live_pilot_expansion_canary_review"])
        self.assertEqual(len([note for note in notes if "live-pilot-expansion-canary" in note.get("tags", [])]), 1)
    ...
    def test_scheduler_runs_review_once_after_close(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                self._seed_notes()
                summary = trade_automation_service.run_trade_automation_live_pilot_expansion_canary_reviews(
                    db,
                    now=FIXED_NOW,
                )
                second_summary = trade_automation_service.run_trade_automation_live_pilot_expansion_canary_reviews(
                    db,
                    now=FIXED_NOW,
                )
        ...
        self.assertEqual(after_state["runtime"]["live_pilot_expansion_canary_last_report"]["run_source"], "scheduled")
    ...
    def test_scheduler_skips_before_close_buffer_and_when_disabled(self) -> None:
        ...
        before_summary = trade_automation_service.run_trade_automation_live_pilot_expansion_canary_reviews(
            db,
            now=BEFORE_CLOSE_NOW,
        )
        state["settings"]["live_pilot_expansion_canary_auto_review_enabled"] = False
        trade_automation_service._write_trade_automation_state(tenant, state, profile_key="personal_paper")
        db.commit()
        disabled_summary = trade_automation_service.run_trade_automation_live_pilot_expansion_canary_reviews(
            db,
            now=FIXED_NOW,
        )
        ...
        self.assertEqual(disabled_summary["items"][0]["reason"], "live_pilot_expansion_canary_auto_review_disabled")

    def test_scheduler_ignores_live_profile(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        paper_state["settings"]["live_pilot_expansion_canary_enabled"] = False
        trade_automation_service._write_trade_automation_state(tenant, paper_state, profile_key="personal_paper")
        ...
        summary = trade_automation_service.run_trade_automation_live_pilot_expansion_canary_reviews(db, now=FIXED_NOW)

        self.assertEqual(summary["processed"], 1)
        self.assertEqual(summary["reviewed"], 0)
        self.assertEqual(summary["items"][0]["profile_key"], "personal_paper")
        live_after = trade_automation_service._read_trade_automation_state(tenant, profile_key="personal_live")
        self.assertFalse(live_after["runtime"].get("live_pilot_expansion_canary_last_report"))
