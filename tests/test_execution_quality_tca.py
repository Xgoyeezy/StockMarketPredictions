...
class ExecutionQualityTcaTests(unittest.TestCase):
    ...
    def test_execution_proof_ready_with_cost_edge_and_fill_quality(self) -> None:
        ...
        for record in proof["record_readiness"]:
            self.assertFalse(record["changes_execution"])
            self.assertFalse(record["changes_order_submission"])
            self.assertFalse(record["changes_broker_routes"])
            self.assertFalse(record["changes_risk_gates"])
            self.assertFalse(record["changes_ranking_weights"])
            self.assertFalse(record["can_change_broker_routes"])
            self.assertFalse(record["can_bypass_risk_gates"])
            self.assertFalse(record["can_change_ranking_weights"])
            self.assertFalse(record["can_grant_ai_order_authority"])
    ...
    def test_execution_quality_hardening_plan_blocks_claims_when_evidence_is_missing(self) -> None:
        ...
        self.assertFalse(any(item["changes_order_submission"] for item in hardening_plan["items"]))
