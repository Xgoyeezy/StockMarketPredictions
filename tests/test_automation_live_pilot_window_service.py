...
class _FakeLiveAdapter:
    ...
    def submit_order(self, *, request, report, live_price, position, trade_id, order_id, order_ticket) -> SubmitOrderResult:
        ...
        row = {
            ...
            "broker_order_id": "live-window-entry-1",
            ...
            "live_pilot_window_id": order_ticket.get("route_correlation_id"),
        }
        if self.mode == "filled":
            ...
            return SubmitOrderResult(
                ...
                broker_response={"id": "live-window-entry-1", "client_order_id": order_id, "symbol": request.ticker, "status": "filled"},
            )
        ...
        return SubmitOrderResult(
            ...
            broker_order_id="live-window-entry-1",
            broker_status="new",
            broker_response={"id": "live-window-entry-1", "client_order_id": order_id, "symbol": request.ticker, "status": "new"},
        )
    ...
    def close_position(self, *, request, target_trade) -> ClosePositionResult:
        ...
        closed = self.ledger.close_trade_by_index(
            ...
            close_fraction=1.0,
            close_updates={"broker_close_order_id": "live-window-close-1", "broker_close_status": "filled"},
        )
        return ClosePositionResult(
            closed_trade=closed,
            broker_name="alpaca_live",
            broker_order_id="live-window-close-1",
            broker_status="filled",
            broker_response={"id": "live-window-close-1", "status": "filled"},
        )


class AutomationLivePilotWindowTests(unittest.TestCase):
    def _db(self):
        engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
        Base.metadata.create_all(engine)
        SessionLocal = sessionmaker(bind=engine, future=True)
        db = SessionLocal()
        tenant = Tenant(slug="live-window-test", name="Live Window Test", status="active")
        db.add(tenant)
        db.commit()
        db.refresh(tenant)
    ...
    def _paper_state(self, *, enabled: bool = True, candidate: dict | None = None) -> dict:
        ...
        state["settings"].update(
            {
                ...
                "live_pilot_window_enabled": enabled,
    ...
    def _patch_io(self, ledger: _Ledger, adapter: _FakeLiveAdapter):
        ...
        return (
            ...
            patch.object(automation_live_pilot_window_service, "get_execution_adapter_for", return_value=adapter),
            patch.object(automation_live_pilot_window_service.sdm, "get_live_price", return_value=500.0),
            patch.object(automation_live_pilot_window_service.sdm, "read_pending_orders", side_effect=ledger.read_pending),
            patch.object(automation_live_pilot_window_service.sdm, "read_open_trades", side_effect=ledger.read_open),
            patch.object(automation_live_pilot_window_service.sdm, "update_pending_order", side_effect=ledger.update_pending),
            patch.object(automation_live_pilot_window_service.sdm, "update_open_trade", side_effect=ledger.update_open),
            patch.object(automation_live_pilot_window_service.sdm, "close_trade_by_index", side_effect=ledger.close_trade_by_index),
            notes_path,
        )

    def test_action_schema_accepts_live_pilot_window_actions(self) -> None:
        self.assertEqual(OrganizationTradeAutomationActionRequest(action="prepare_live_pilot_window").action, "prepare_live_pilot_window")
        self.assertEqual(OrganizationTradeAutomationActionRequest(action="run_live_pilot_window_entry").action, "run_live_pilot_window_entry")
        self.assertEqual(OrganizationTradeAutomationActionRequest(action="run_live_pilot_window_exit").action, "run_live_pilot_window_exit")

    def test_disabled_window_blocks_prepare_without_order_path(self) -> None:
        db, tenant = self._db()
        ...
        *context_patches, notes_path = patches
        ...
        with context_patches[0], context_patches[1], context_patches[4], context_patches[5], patch.object(
            automation_live_pilot_window_service,
        ...
        self.assertIn("live_pilot_window_disabled", {item["key"] for item in report["blockers"]})
        self.assertEqual(adapter.submitted, 0)
        self.assertEqual(len([note for note in notes if "supervised-live-pilot" in note.get("tags", [])]), 1)
    ...
    def test_prepare_creates_fresh_approval_note_and_audit(self) -> None:
        ...
        *context_patches, notes_path = patches
        ...
        with context_patches[0], context_patches[1], context_patches[4], context_patches[5]:
            report = automation_live_pilot_window_service.prepare_live_pilot_window(
                db,
                tenant=tenant,
                paper_state=paper_state,
        ...
        self.assertEqual(adapter.submitted, 0)
        self.assertEqual(paper_state["runtime"]["live_pilot_window_approval"]["status"], "approved")
        self.assertEqual(report["selected_candidate"]["ticker"], "SPY")
        self.assertEqual(len([note for note in notes if "supervised-live-pilot" in note.get("tags", [])]), 1)
        audit_types = [row.event_type for row in db.query(AuditEvent).all()]
        self.assertIn("trade_automation.live_pilot_window_prepared", audit_types)

    def test_blocked_expansion_canary_blocks_prepare(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        paper_state["runtime"]["live_pilot_expansion_canary_last_report"]["status"] = "blocked"
        ledger = _Ledger()
        adapter = _FakeLiveAdapter(ledger)
        patches = self._patch_io(ledger, adapter)
        *context_patches, _notes_path = patches
        ...
        self.assertEqual(report["status"], "blocked")
        self.assertIn("live_pilot_expansion_canary_not_ready", {item["key"] for item in report["blockers"]})
        self.assertEqual(adapter.submitted, 0)

    def test_stale_approval_blocks_entry_without_submit(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        paper_state["runtime"]["live_pilot_window_approval"] = {
            "approval_id": "old-approval",
            "status": "approved",
            "expires_at": (FIXED_NOW - timedelta(minutes=1)).isoformat(),
            "window_expires_at": (FIXED_NOW + timedelta(minutes=60)).isoformat(),
            "selected_candidate": {"ticker": "SPY"},
            "symbol": "SPY",
            "notional_cap": 50,
            "session_order_cap": 1,
        }
    ...
    def test_no_clean_candidate_blocks_prepare(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state(candidate={"ticker": "SPY", "auto_entry_eligible": False, "reject_reason": "spread"})
        ledger = _Ledger()
        adapter = _FakeLiveAdapter(ledger)
        patches = self._patch_io(ledger, adapter)
        *context_patches, _notes_path = patches
    ...
    def test_entry_working_then_explicit_cancel_records_terminal_evidence(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        ledger = _Ledger()
        ...
        *context_patches, notes_path = patches
        ...
        with context_patches[0], context_patches[1], context_patches[4], context_patches[5]:
            automation_live_pilot_window_service.prepare_live_pilot_window(
                db,
                tenant=tenant,
                paper_state=paper_state,
                live_state=self._live_state(),
        ...
        with ExitStack() as stack:
            for patcher in context_patches[:9]:
                stack.enter_context(patcher)
            entry_report = automation_live_pilot_window_service.run_live_pilot_window_entry(
                db,
                tenant=tenant,
                paper_state=paper_state,
                live_state=self._live_state(),
            ...
            exit_report = automation_live_pilot_window_service.run_live_pilot_window_exit(
                db,
                tenant=tenant,
                paper_state=paper_state,
                live_state=self._live_state(),
        ...
        self.assertEqual(entry_report["status"], "entered")
        self.assertEqual(entry_report["terminal_state"], "working")
        self.assertEqual(exit_report["status"], "completed")
        self.assertEqual(exit_report["terminal_state"], "canceled")
        self.assertEqual(exit_report["reconciliation_status"], "clean")
        self.assertEqual(adapter.submitted, 1)
        self.assertEqual(adapter.canceled, 1)
        self.assertFalse(ledger.pending)
        self.assertEqual(len([note for note in notes if "supervised-live-pilot" in note.get("tags", [])]), 1)

    def test_entry_filled_then_explicit_close_records_terminal_evidence(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        ledger = _Ledger()
        ...
        *context_patches, _notes_path = patches
        ...
        self.assertEqual(entry_report["status"], "entered")
        self.assertEqual(entry_report["terminal_state"], "open")
        self.assertEqual(exit_report["status"], "completed")
        self.assertEqual(exit_report["terminal_state"], "closed")
        self.assertEqual(exit_report["reconciliation_status"], "clean")
        self.assertEqual(adapter.submitted, 1)
    ...
    def test_action_path_prepare_persists_snapshot_without_live_gate_mutation(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        live_state = self._live_state()
        ...
        *context_patches, _notes_path = patches
        current_user = SimpleNamespace(
            ...
            auth_subject="live-window-action",
            user_id="live-window-action",
        ...
        with (
            ...
            snapshot = trade_automation_service.run_tenant_trade_automation_action(
                ...
                request=OrganizationTradeAutomationActionRequest(action="prepare_live_pilot_window"),
            )
        ...
        self.assertEqual(snapshot["live_pilot_window"]["status"], "approved")
        self.assertTrue(snapshot["available_actions"]["can_prepare_live_pilot_window"])
        self.assertTrue(snapshot["available_actions"]["can_run_live_pilot_window_entry"])
        self.assertEqual(adapter.submitted, 0)

    def test_action_path_entry_and_exit_persist_without_live_state_mutation(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        live_state = self._live_state()
        trade_automation_service._write_trade_automation_state(tenant, paper_state, profile_key="personal_paper")
        ...
        *context_patches, notes_path = patches
        ...
        with (
            ...
            entry_snapshot = trade_automation_service.run_tenant_trade_automation_action(
                db,
                current_user=current_user,
                request=OrganizationTradeAutomationActionRequest(action="run_live_pilot_window_entry"),
            )
            exit_snapshot = trade_automation_service.run_tenant_trade_automation_action(
                db,
                current_user=current_user,
                request=OrganizationTradeAutomationActionRequest(action="run_live_pilot_window_exit"),
            )
            notes = json.loads(notes_path.read_text(encoding="utf-8"))

        after_live_state = trade_automation_service._read_trade_automation_state(tenant, profile_key="personal_live")
        self.assertEqual(after_live_state["settings"], before_live_settings)
        self.assertEqual(entry_snapshot["live_pilot_window"]["status"], "entered")
        self.assertEqual(entry_snapshot["live_pilot_window"]["terminal_state"], "working")
        self.assertEqual(exit_snapshot["live_pilot_window"]["status"], "completed")
        self.assertEqual(exit_snapshot["live_pilot_window"]["terminal_state"], "canceled")
        self.assertEqual(adapter.submitted, 1)
        self.assertEqual(adapter.canceled, 1)
        self.assertEqual(len([note for note in notes if "supervised-live-pilot" in note.get("tags", [])]), 1)
