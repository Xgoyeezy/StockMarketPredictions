...
class PortfolioRiskIntelligenceTests(unittest.TestCase):
    ...
    def test_portfolio_risk_proof_ready_with_full_context(self) -> None:
        ...
        self.assertFalse(any(row["changes_order_submission"] for row in proof["requirements"]))
    ...
    def test_portfolio_risk_cleanup_plan_blocks_claims_when_context_is_missing(self) -> None:
        ...
        self.assertFalse(any(item["changes_order_submission"] for item in cleanup_plan["items"]))
        self.assertFalse(any(item["changes_broker_routes"] for item in cleanup_plan["items"]))
        self.assertFalse(any(item["changes_risk_gates"] for item in cleanup_plan["items"]))
        self.assertFalse(any(item["changes_ranking_weights"] for item in cleanup_plan["items"]))
        self.assertFalse(any(item["can_change_broker_routes"] for item in cleanup_plan["items"]))
        self.assertFalse(any(item["can_bypass_risk_gates"] for item in cleanup_plan["items"]))
        self.assertFalse(any(item["can_change_ranking_weights"] for item in cleanup_plan["items"]))
        self.assertFalse(any(item["can_grant_ai_order_authority"] for item in cleanup_plan["items"]))
        for action in cleanup_plan["safe_next_actions"]:
    ...
    def test_portfolio_risk_cleanup_plan_helper_is_read_only(self) -> None:
        ...
        self.assertFalse(cleanup_plan["can_submit_live_orders"])
        self.assertFalse(cleanup_plan["can_change_broker_routes"])
        self.assertFalse(cleanup_plan["can_bypass_risk_gates"])
        self.assertFalse(cleanup_plan["can_clear_kill_switch"])
        self.assertFalse(cleanup_plan["can_change_ranking_weights"])
        self.assertFalse(cleanup_plan["can_grant_ai_order_authority"])
