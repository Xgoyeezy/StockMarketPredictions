...
FIXED_NOW = datetime(2026, 4, 24, 20, 55, tzinfo=timezone.utc)
...
class _FakeLiveAdapter:
    ...
    def submit_order(self, *, request, report, live_price, position, trade_id, order_id, order_ticket) -> SubmitOrderResult:
        ...
        row = {
            ...
            "broker_order_id": "live-expansion-entry-1",
        ...
        if self.mode == "filled":
            ...
            return SubmitOrderResult(
                ...
                broker_response={"id": "live-expansion-entry-1", "client_order_id": order_id, "symbol": request.ticker, "status": "filled"},
            )
        ...
        return SubmitOrderResult(
            ...
            broker_order_id="live-expansion-entry-1",
            broker_status="new",
            broker_response={"id": "live-expansion-entry-1", "client_order_id": order_id, "symbol": request.ticker, "status": "new"},
        )
    ...
    def close_position(self, *, request, target_trade) -> ClosePositionResult:
        ...
        closed = self.ledger.close_trade_by_index(
            ...
            close_fraction=1.0,
            close_updates={"broker_close_order_id": "live-expansion-close-1", "broker_close_status": "filled"},
        )
        return ClosePositionResult(
            closed_trade=closed,
            broker_name="alpaca_live",
            broker_order_id="live-expansion-close-1",
            broker_status="filled",
            broker_response={"id": "live-expansion-close-1", "status": "filled"},
        )


class AutomationLivePilotExpansionTests(unittest.TestCase):
    def _db(self):
        engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
        Base.metadata.create_all(engine)
        SessionLocal = sessionmaker(bind=engine, future=True)
        db = SessionLocal()
        tenant = Tenant(slug="live-expansion-test", name="Live Expansion Test", status="active")
        db.add(tenant)
        db.commit()
        db.refresh(tenant)
        ...
        state["settings"].update(
            {
                ...
                "live_pilot_expansion_enabled": enabled,
    ...
    def _patch_io(self, ledger: _Ledger, adapter: _FakeLiveAdapter, *, sync_state: str = "working"):
        ...
        return (
            ...
            patch.object(automation_live_pilot_expansion_service, "get_execution_adapter_for", return_value=adapter),
            patch.object(automation_live_pilot_expansion_service, "sync_pending_orders_from_broker", return_value=sync_payload),
            patch.object(automation_live_pilot_expansion_service.sdm, "get_live_price", return_value=500.0),
            patch.object(automation_live_pilot_expansion_service.sdm, "read_pending_orders", side_effect=ledger.read_pending),
            patch.object(automation_live_pilot_expansion_service.sdm, "read_open_trades", side_effect=ledger.read_open),
            patch.object(automation_live_pilot_expansion_service.sdm, "update_pending_order", side_effect=ledger.update_pending),
            patch.object(automation_live_pilot_expansion_service.sdm, "update_open_trade", side_effect=ledger.update_open),
            patch.object(automation_live_pilot_expansion_service.sdm, "close_trade_by_index", side_effect=ledger.close_trade_by_index),
            notes_path,
        )

    def test_action_schema_accepts_live_pilot_expansion_actions(self) -> None:
        prepare = OrganizationTradeAutomationActionRequest(action="prepare_live_pilot_expansion")
        run = OrganizationTradeAutomationActionRequest(action="run_live_pilot_expansion")

        self.assertEqual(prepare.action, "prepare_live_pilot_expansion")
        self.assertEqual(run.action, "run_live_pilot_expansion")

    def test_disabled_expansion_blocks_prepare_without_order_path(self) -> None:
        db, tenant = self._db()
        ...
        *context_patches, notes_path = patches
        ...
        self.assertIn("live_pilot_expansion_disabled", {item["key"] for item in report["blockers"]})
        self.assertEqual(adapter.submitted, 0)
        self.assertEqual(len([note for note in notes if "live-pilot-expansion" in note.get("tags", [])]), 1)
    ...
    def test_prepare_creates_fresh_approval_note_and_audit(self) -> None:
        ...
        *context_patches, notes_path = patches
        ...
        self.assertEqual(adapter.submitted, 0)
        self.assertEqual(paper_state["runtime"]["live_pilot_expansion_approval"]["status"], "approved")
        self.assertEqual(report["selected_candidate"]["ticker"], "SPY")
        self.assertEqual(len([note for note in notes if "live-pilot-expansion" in note.get("tags", [])]), 1)
        audit_types = [row.event_type for row in db.query(AuditEvent).all()]
        self.assertIn("trade_automation.live_pilot_expansion_prepared", audit_types)

    def test_stale_approval_blocks_run_without_submit(self) -> None:
        db, tenant = self._db()
        paper_state = self._paper_state()
        paper_state["runtime"]["live_pilot_expansion_approval"] = {
    ...
    def test_clean_submit_cancel_path_records_terminal_evidence(self) -> None:
        ...
        *context_patches, notes_path = patches
        current_user = SimpleNamespace(tenant_id=tenant.id, tenant_slug=tenant.slug, auth_subject="live-expansion", user_id="live-expansion")

        with context_patches[0], context_patches[1]:
            automation_live_pilot_expansion_service.prepare_live_pilot_expansion(
                db,
                tenant=tenant,
                paper_state=paper_state,
                live_state=self._live_state(),
        ...
        with ExitStack() as stack:
            for patcher in context_patches[:10]:
    ...
    def test_unexpected_fill_closes_and_reconciles(self) -> None:
        ...
        *context_patches, _notes_path = patches
        ...
        with ExitStack() as stack:
            for patcher in (context_patches[0], context_patches[1], context_patches[2], context_patches[4], context_patches[5], context_patches[6], context_patches[7], context_patches[8], context_patches[9]):
                stack.enter_context(patcher)
            report = automation_live_pilot_expansion_service.run_live_pilot_expansion(
                db,
                tenant=tenant,
    ...
    def test_action_path_persists_snapshot_without_live_gate_mutation(self) -> None:
        ...
        *context_patches, _notes_path = patches
        current_user = SimpleNamespace(
            ...
            auth_subject="live-expansion-action",
            user_id="live-expansion-action",
        ...
        with (
            ...
            snapshot = trade_automation_service.run_tenant_trade_automation_action(
                db,
                current_user=current_user,
                request=OrganizationTradeAutomationActionRequest(action="prepare_live_pilot_expansion"),
            )
        ...
        self.assertEqual(snapshot["live_pilot_expansion"]["status"], "approved")
        self.assertTrue(snapshot["available_actions"]["can_prepare_live_pilot_expansion"])
        self.assertTrue(snapshot["available_actions"]["can_run_live_pilot_expansion"])
        self.assertEqual(adapter.submitted, 0)
